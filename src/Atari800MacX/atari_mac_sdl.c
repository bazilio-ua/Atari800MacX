...
extern void SoundManagerStereoDo(void);
extern void SoundManagerRecordingDo(void);
extern void SoundManagerIncreaseVolume(void);
extern void SoundManagerDecreaseVolume(void);
...
int joystick0extraNum;
int joystick1extraNum;
int joystick2extraNum;
int joystick3extraNum;
...
void Check_SDL_Joysticks()
{
    if (joystick0 != NULL) {
		...
		// EER1 -- retrolink USB Atari joystick fix
		int numAxes = SDL_JoystickNumAxes(joystick0);
		joystick0extraNum = numAxes % 2;
		joystick0_nsticks = numAxes / 2;
//        joystick0_nsticks = SDL_JoystickNumAxes(joystick0)/2;
    ...
    if (joystick1 != NULL) {
		...
		// EER1 -- retrolink USB Atari joystick fix
		int numAxes = SDL_JoystickNumAxes(joystick1);
		joystick1extraNum = numAxes % 2;
		joystick1_nsticks = numAxes / 2;
//        joystick1_nsticks = SDL_JoystickNumAxes(joystick1)/2;
    ...
    if (joystick2 != NULL) {
		...
		// EER1 -- retrolink USB Atari joystick fix
		int numAxes = SDL_JoystickNumAxes(joystick2);
		joystick2extraNum = numAxes % 2;
		joystick2_nsticks = numAxes / 2;
//        joystick2_nsticks = SDL_JoystickNumAxes(joystick2)/2;
    ...
    if (joystick3 != NULL) {
		...
		// EER1 -- retrolink USB Atari joystick fix
		int numAxes = SDL_JoystickNumAxes(joystick3);
		joystick3extraNum = numAxes % 2;
		joystick3_nsticks = numAxes / 2;
//        joystick3_nsticks = SDL_JoystickNumAxes(joystick3)/2;
        joystick3_nhats = SDL_JoystickNumHats(joystick3);
        if (joystick3TypePref == JOY_TYPE_STICK) {
...
*-----------------------------------------------------------------------------*/
// EER1 -- retrolink USB Atari joystick fix
int get_SDL_joystick_state(SDL_Joystick *joystick, int stickNum, int extraNum, int *x_pot, int *y_pot)
//int get_SDL_joystick_state(SDL_Joystick *joystick, int stickNum, int *x_pot, int *y_pot)
{
    int x;
    int y;

    x = SDL_JoystickGetAxis(joystick, stickNum*2 + extraNum);
    y = SDL_JoystickGetAxis(joystick, stickNum*2+1 + extraNum);
            ...
            case SDL0_JOYSTICK:
                if (joystickType0 == JOY_TYPE_STICK) {
					if (joystick0Num == 9999) {
						stick[i] = get_SDL_joystick_state(joystick0, joy0_multi_num, 0,
														  &joy_x_pot[i], &joy_y_pot[i]);
						joy0_multi_num = (joy0_multi_num + 1) % joystick0_nsticks;
					} else {
						stick[i] = get_SDL_joystick_state(joystick0, joystick0Num, joystick0extraNum,
														  &joy_x_pot[i], &joy_y_pot[i]);
						}
				}
					...
					if (joystick0Num == 9999) {
						...
						joy0_multi_num = (joy0_multi_num + 1) % joystick0_nhats;
					} else {
            ...
            case SDL1_JOYSTICK:
                if (joystickType1 == JOY_TYPE_STICK) {
					if (joystick1Num == 9999) {
						stick[i] = get_SDL_joystick_state(joystick1, joy1_multi_num, 0,
														  &joy_x_pot[i], &joy_y_pot[i]);
						joy1_multi_num = (joy1_multi_num + 1) % joystick1_nsticks;
					} else {
						stick[i] = get_SDL_joystick_state(joystick1, joystick1Num, joystick1extraNum,
														  &joy_x_pot[i], &joy_y_pot[i]);
						}
				}
            ...
            case SDL2_JOYSTICK:
				if (joystickType2 == JOY_TYPE_STICK) {
					if (joystick2Num == 9999) {
						stick[i] = get_SDL_joystick_state(joystick2, joy2_multi_num, 0,
														  &joy_x_pot[i], &joy_y_pot[i]);
						joy2_multi_num = (joy2_multi_num + 1) % joystick2_nsticks;
					} else {
						stick[i] = get_SDL_joystick_state(joystick2, joystick2Num, joystick2extraNum,
														  &joy_x_pot[i], &joy_y_pot[i]);
						}
				}
            ...
            case SDL3_JOYSTICK:
				if (joystickType3 == JOY_TYPE_STICK) {
					if (joystick3Num == 9999) {
						stick[i] = get_SDL_joystick_state(joystick3, joy3_multi_num, 0,
														  &joy_x_pot[i], &joy_y_pot[i]);
						joy3_multi_num = (joy3_multi_num + 1) % joystick3_nsticks;
					} else {
						stick[i] = get_SDL_joystick_state(joystick3, joystick3Num, joystick3extraNum,
														  &joy_x_pot[i], &joy_y_pot[i]);
					}
				}
        ...
        switch(JOYSTICK_MODE[i]) {
            ...
            case SDL2_JOYSTICK:
				if (joystick0Num == 9999) {
					...
					joy2_multi_num = (joy2_multi_num + 1) % (MAX(joystick2_nsticks,joystick2_nhats));
				} else {
					trig[i] = get_SDL_button_state(joystick2, joystick2_nbuttons, 
												   &key_code2, INPUT_key_shift, key_control,
												   key_consol,0);
